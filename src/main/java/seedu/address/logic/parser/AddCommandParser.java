...
public class AddCommandParser implements Parser<AddCommand> {
    ...
    public AddCommand parse(String args) throws ParseException {
        ...
        if (!arePrefixesPresent(argMultimap, PREFIX_NAME, PREFIX_ADDRESS, PREFIX_STUDENT_ID, PREFIX_EMAIL)
                || !argMultimap.getPreamble().isEmpty()) {
            throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT, AddCommand.MESSAGE_USAGE));
        }

        argMultimap.verifyNoDuplicatePrefixesFor(PREFIX_NAME, PREFIX_STUDENT_ID, PREFIX_EMAIL, PREFIX_ADDRESS);
        Name name = ParserUtil.parseName(argMultimap.getValue(PREFIX_NAME).get());
        StudentId studentId = ParserUtil.parsePhone(argMultimap.getValue(PREFIX_STUDENT_ID).get());
        EmailId emailId = ParserUtil.parseEmail(argMultimap.getValue(PREFIX_EMAIL).get());
